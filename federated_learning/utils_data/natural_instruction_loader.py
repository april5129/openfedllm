...
def _get_task_splits():
    base_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), 'datasets', 'natural-instructions-2.8')
    with open(os.path.join(base_path, 'splits', 'default', 'train_tasks.txt'), 'r') as reader:
        train_set_names = [f'{content.strip()}.json' for content in reader.readlines()]
    with open(os.path.join(base_path, 'splits', 'default', 'test_tasks.txt'), 'r') as reader:
...
def get_instruction_dataset(args, tokenizer, only_eval=False):
    ...
    if not only_eval:
        ...
        for file_name in train_set_names:
            with open(os.path.join(base_path, 'tasks', file_name)) as reader:
                ...
                instances = np.random.choice(instances, 10, replace=False)
    ...
    for file_name in eval_set_names:
        with open(os.path.join(base_path, 'tasks', file_name)) as reader:
            ...
            if len(instances) > 20:
                ...
                instances = np.random.choice(instances, max(20, 5), replace=False)
            data = []
            for item in instances:
                # only take the first output into consideration
                data.append((instruct, item['input'], item['output'][0]))
